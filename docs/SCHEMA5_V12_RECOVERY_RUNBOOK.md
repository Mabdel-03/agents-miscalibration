# Schema-5 v1.2-r8 recovery and production runbook
...
release is `sweep-recovery-schema5-v1.2-r8`, the logical release is
`sweep-recovery-schema5-v1.2`, and the chain namespace is `schema5-v1.2-r8`.
...
tag=sweep-recovery-schema5-v1.2-r8
release_id=sweep-recovery-schema5-v1.2
slurm_user=mabdel03
chain_manifest="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R8.json"
pilot_checkout="$recovery/materialization_pilot_source_checkout_v1_2_r8"
pilot_root="$recovery/materialization_pilots/schema5-v1.2-r8"
canary_root="$recovery/slurm_canaries/schema5-v1.2-r8"
dev_python="$(realpath -e /orcd/home/002/mabdel03/conda_envs/asys_env/bin/python)"
conda_toolchain_namespace="$recovery/toolchains/r8"
...
durable_commit_ref=refs/heads/schema5-v1.2-r8
durable_marker="$recovery/DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R8_COMPLETE.json"
...
evidence. Do not move, overwrite, delete, or republish any of them under an r8 name.
...
The r8 renderer must verify and bind the exact seal path, raw SHA-256, size, and
...
seal fails closed. All r8 pilot, canary, source-checkout, job/log, protected-capacity,
repair, and sentinel paths are fresh r8 paths; no r2 completion marker is copied
forward.
...
client-capacity, and other active wire payload uses a `schema5-v1.2-r8-*`
protocol, an r8-specific artifact path, and binds the r8 tag and
`chain_namespace=schema5-v1.2-r8` wherever that chain namespace is part of the
payload schema. Operational artifact basenames are r8-specific; r2 names appear
...
The shared runtime must remain untouched and must never be executed by r8.

The r8-tagged sealer archives the two canonical probe envelopes marker-first under
...
tagged r8 checkout and toolchain are created below; there is intentionally no
...
After the r8 tag and detached pilot checkout exist, the tagged r8 failure sealer
...
`$r4_toolchain_failure_marker` last. The r8 renderer and every sentinel bind and
...
After the r8 tag exists, the tagged r8 sealer reproduces that exact r5 CLI invocation
...
toolchain, and publishes `$r5_prelaunch_failure_marker` last. The r8 renderer and
...
After the r8 tag exists, the tagged r8 sealer reproduces that exact r6 dry-run
...
The r8 recorder accepts and validates the complete returned binding, including the
portable-shebang contract. The r8 renderer and every sentinel reverify the r4, r5,
...
After the r8 tag exists, the tagged r8 sealer clones the exact durable r7 bundle,
...
The r8 recorder sets `GIT_OPTIONAL_LOCKS=0` only for its read-only Git identity
queries. The r8 renderer and every sentinel independently bind and reverify the r7
...
The r8 production prerequisite and environment-capture job accept only Conda
...
the distinct `release_source_checkout_v1_2_r8` path and requires that path to be
...
Seal the deterministic r4 toolchain failure from the exact tagged r8 checkout before
...
Seal the deterministic r5 prelaunch CLI failure from the exact tagged r8 checkout.
...
r8 checkout. The dry run validates the one-field mismatch without executing the
...
r8 checkout. The dry run validates all historical inputs without cloning or running
...
Provision and verify the r8-only Conda toolchain from the exact tagged checkout.
...
Follow [SCHEMA5_R8_CONDA_TOOLCHAIN.md](SCHEMA5_R8_CONDA_TOOLCHAIN.md) for the complete
link, inode, offline-probe, and crash-quarantine contract.

Produce the two r3 prelaunch-failure envelopes only after the r8 toolchain above has
...
the tagged r8 provisioner in-process. That provisioner performs the complete
marker/runtime/inventory binding without executing the sealed toolchain; only after
that independent check may the exact probe command launch the sealed r8 Python or
Conda executable.
...
`85e198c5…8d863a`) with the sealed r8 Python, `-I`, and `-B`. Its
`conda-runtime-identity` subcommand reads, but never executes, the recorded shared
Conda base. The offline probe executes only the sealed r8 Conda and clones that same
...
```bash
r8_probe_python="$conda_toolchain_root/base/bin/python"
r8_probe_conda="$conda_toolchain_root/base/bin/conda"
r8_sealer="$pilot_checkout/scripts/seal_recovery_evidence.py"
...
r3_probe_root="/tmp/schema5-r3-prelaunch-${slurm_user}-r8"
...
r3_broken_probe=(
  "$dev_python" -I -B "$r8_sealer" record-prelaunch-attempt
  ...
  --input-root tagged-r8-release-checkout "$pilot_checkout"
  --input-root sealed-r8-conda-toolchain "$conda_toolchain_root"
  --input-root recorded-shared-conda-base "$recorded_shared_conda_base"
  --write-root "$r3_probe_root"
)
r3_broken_command=(
  "$r8_probe_python" -I -B "$r3_pilot"
...
r3_offline_command=(
  "$r8_probe_conda" create --yes --offline
...
r3_prelaunch_seal=(
  "$dev_python" -I -B "$r8_sealer" seal-prelaunch-failure
...
rerunning either failure. The recorder inventories the tagged r3 checkout, tagged r8
checkout, sealed r8 toolchain, and—only for the runtime-identity probe—the recorded
...
sealed root and the exact tagged r8 verifier. This is a hard render prerequisite:

```bash
"$dev_python" -I -B "$r8_sealer" \
...
pilot_sbatch="$recovery/jobs/schema5-v1.2-r8-materialization-pilot.sbatch"
pilot_receipt="$pilot_sbatch.receipt.json"
pilot_logs="$recovery/logs/materialization-pilot-r8"
...
protected_canary_root="$recovery/protected_capacity/schema5-v1.2-r8"
...
effective_fleet_root="$recovery/effective-fleet-schema5-v1.2-r8"
...
`schema5-v1.2-r8-effective-fleet-materialization-v1`. Its create-once intent binds
...
`schema5-v1.2-r8` namespace. It uses a semantic canonical-JSON `marker_id`; the
...
`schema5-v1.2-r8-protected-capacity-v4` and capacity source
...
protocol `schema5-v1.2-r8-external-watchdog-v1`, additionally binds a
...
watchdog_vm_stage=/var/tmp/schema5-watchdog-v1.2-r8
...
After publishing the annotated r8 tag, exact remote branch, and durable Git marker,
but before rendering the 43-job chain, require all of the following:

- the clean detached r8 pilot checkout resolves to the exact annotated tag object and
...
- the genuine r8 composite Slurm canary verifies at its fresh canonical root;
...
pre-render because they bind the annotated r8 tag and commit. Bootstrap-watchdog
...
chain_receipt="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R8_SUBMISSION.json"
...
launch_complete="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R8_LAUNCHED.json"
...
bootstrap_vm_stage=/var/tmp/schema5-bootstrap-watchdog-v1.2-r8
...
isolated_manifest="$isolated_root/RECOVERY_CHAIN_SCHEMA5_V1_2_R8.json"
isolated_receipt="$isolated_root/RECOVERY_CHAIN_SCHEMA5_V1_2_R8_SUBMISSION.json"
...
' "$isolated_receipt"
test ! -e "$recovery/recovery_chain_repairs_v1_2_r8"
test ! -e "$root_release"
...
`RECOVERY_CHAIN_SCHEMA5_V1_2_R8_LAUNCHED.json` is published last. A crash after
...
`recovery_chain_stage_sentinels/schema5-v1.2-r8/gNNNN/<stage>/`. A stage observation
...
`schema5-v1.2-r8-bootstrap-watchdog-handoff-v1`, binding the generation, receipt,
...
marker are required
pre-render inputs at their canonical r8 paths. Rendering fails if any of those
...
read-only and publishes `SOURCE_CHECKOUT_SCHEMA5_V1_2_R8_COMPLETE.json` last; jobs
...
The r8 controller implements the complete `24 -> 96 -> 192 -> 384` rollout state
...
`schema5-v1.2-r8-client-placement-capacity-generation-v1`; it binds the immutable
...
r8 release/scientific identities, the original sealed failure and receipt, the
