# Schema-5 v1.2-r15 recovery and production runbook
...
release is `sweep-recovery-schema5-v1.2-r15`, the logical release is
`sweep-recovery-schema5-v1.2`, and the chain namespace is `schema5-v1.2-r15`.
...
tag=sweep-recovery-schema5-v1.2-r15
release_id=sweep-recovery-schema5-v1.2
slurm_user=mabdel03
chain_manifest="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R15.json"
pilot_checkout="$recovery/materialization_pilot_source_checkout_v1_2_r15"
pilot_root="$recovery/materialization_pilots/schema5-v1.2-r15"
canary_root="$recovery/slurm_canaries/schema5-v1.2-r15"
dev_python="$(realpath -e /orcd/home/002/mabdel03/conda_envs/asys_env/bin/python)"
conda_toolchain_namespace="$recovery/toolchains/r15"
...
durable_commit_ref=refs/heads/schema5-v1.2-r15
durable_marker="$recovery/DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R15_COMPLETE.json"
...
r9_prelaunch_failure_root="$recovery/prelaunch_failures/schema5-v1.2-r9-offline-diagnostic"
r9_prelaunch_failure_marker="$r9_prelaunch_failure_root/PRELAUNCH_OFFLINE_DIAGNOSTIC_FAILURE_SEALED.json"
r10_prelaunch_failure_root="$recovery/prelaunch_failures/schema5-v1.2-r10-r9-sealer"
r10_prelaunch_failure_marker="$r10_prelaunch_failure_root/PRELAUNCH_R9_SEALER_FAILURE_SEALED.json"
r11_prelaunch_failure_root="$recovery/prelaunch_failures/schema5-v1.2-r11-r9-sealer"
r11_prelaunch_failure_marker="$r11_prelaunch_failure_root/PRELAUNCH_EQUIVALENT_DIAGNOSTIC_FAILURE_SEALED.json"
r12_prelaunch_failure_root="$recovery/prelaunch_failures/schema5-v1.2-r12-r9-verifier"
r12_prelaunch_failure_marker="$r12_prelaunch_failure_root/PRELAUNCH_R9_VERIFIER_FAILURE_SEALED.json"
r13_prelaunch_failure_root="$recovery/prelaunch_failures/schema5-v1.2-r13-r3-offline-delimiter"
r13_prelaunch_failure_marker="$r13_prelaunch_failure_root/PRELAUNCH_R3_OFFLINE_DELIMITER_FAILURE_SEALED.json"
storage_admission_minimum_free_bytes=7696581394432
```
...
The operator-authorized pre-qualification threshold is 7 TiB
(`7696581394432` bytes) free on the results filesystem. This replaces the earlier
8 TiB operational reserve so qualification can begin promptly. It does not weaken
the frozen live-monitor emergency floor: production still pauses before either
500 GiB free or 10% filesystem free is crossed, and still requires at least one
million and 5% free inodes.

Run this read-only check immediately before the first qualification submission and
again before rendering and submitting the held chain:

```bash
monitoring_contract="$repo/configs/schema5_monitoring.v1.json"
test "$(jq -er '.alerts.prequalification_minimum_disk_free_bytes' \
  "$monitoring_contract")" = "$storage_admission_minimum_free_bytes"
storage_free_bytes="$(df -B1 --output=avail "$results" | awk 'NR == 2 {print $1}')"
[[ "$storage_free_bytes" =~ ^[0-9]+$ ]]
test "$storage_free_bytes" -ge "$storage_admission_minimum_free_bytes"
storage_free_inodes="$(df --output=iavail "$results" | awk 'NR == 2 {print $1}')"
storage_inode_use_percent="$(df --output=ipcent "$results" |
  awk 'NR == 2 {gsub(/%/, "", $1); print $1}')"
[[ "$storage_free_inodes" =~ ^[0-9]+$ ]]
[[ "$storage_inode_use_percent" =~ ^[0-9]+$ ]]
test "$storage_free_inodes" -ge 1000000
test "$storage_inode_use_percent" -le 95
```

Before stage 20 resumes production, use the measured bytes/QID by stratum to project
remaining raw data, checkpoints/logs, and the final snapshot. Keep the larger of the
live 10%-free floor and the measured projection reserve. A threatened live floor
still pauses and drains admission before storage expansion or archival; the lower
pre-qualification threshold is not permission to run through that emergency stop.

## Immutable r2–r13 to r15 lineage
...
evidence. Do not move, overwrite, delete, or republish any of them under an r15 name.
...
The r15 renderer must verify and bind the exact seal path, raw SHA-256, size, and
...
seal fails closed. All r15 pilot, canary, source-checkout, job/log, protected-capacity,
repair, and sentinel paths are fresh r15 paths; no r2 completion marker is copied
forward.
...
client-capacity, and other active wire payload uses a `schema5-v1.2-r15-*`
protocol, an r15-specific artifact path, and binds the r15 tag and
`chain_namespace=schema5-v1.2-r15` wherever that chain namespace is part of the
payload schema. Operational artifact basenames are r15-specific; r2 names appear
...
The shared runtime must remain untouched and must never be executed by r15.

The r15-tagged sealer archives the two canonical probe envelopes marker-first under
...
tagged r15 checkout and toolchain are created below; there is intentionally no
...
After the r15 tag and detached pilot checkout exist, the tagged r15 failure sealer
...
`$r4_toolchain_failure_marker` last. The r15 renderer and every sentinel bind and
...
After the r15 tag exists, the tagged r15 sealer reproduces that exact r5 CLI invocation
...
toolchain, and publishes `$r5_prelaunch_failure_marker` last. The r15 renderer and
...
After the r15 tag exists, the tagged r15 sealer reproduces that exact r6 dry-run
...
The r15 recorder accepts and validates the complete returned binding, including the
portable-shebang contract. The r15 renderer and every sentinel reverify the r4, r5,
...
After the r15 tag exists, the tagged r15 sealer clones the exact durable r7 bundle,
...
The r15 recorder sets `GIT_OPTIONAL_LOCKS=0` only for its read-only Git identity
queries. The r15 renderer and every sentinel independently bind and reverify the r7
...
After the r15 tag exists, the tagged r15 sealer reproduces the exact outer r8 recorder
...
The r15 recorder launches the pinned r3 stdlib-only
`schema5_conda_runtime_identity.py` directly, avoiding the unrelated pilot import
graph while preserving the exact historical runtime-identity contract. The r15
renderer and every sentinel bind and reverify the r8 seal with the earlier history.

The immutable r9 release is commit
`f134bba109ce77beca60584a5f589361856f2301`, annotated tag object
`17b0386800c70fb9e72d090c7b8e1c3ddf9c7b52`, tag
`sweep-recovery-schema5-v1.2-r9`, and namespace `schema5-v1.2-r9`. Its durable
bundle and isolated short-prefix Conda toolchain completed and independently verify
read-only. Its corrected direct broken-link probe published the canonical r3
envelope with exact, unchanged input inventories. The subsequent empty-cache offline
clone failed as expected before scheduler submission, but the r9 recorder rejected
the genuine Conda 25.11 diagnostic: r9 required empty stdout and exactly one
`OfflineError` block, while the pinned Conda emits a bounded clone/progress stream on
stdout and one or more canonical offline-fetch blocks on stderr in nondeterministic
order and cardinality. The failed command left only its declared `/tmp` partial cache
and destination scaffold; it published no offline envelope, r3 seal, scheduler job,
schema-5 run root, pool, controller state, or scientific result. A pre-tag live dry
audit recorded the 18-entry volatile tree, accepted broken-envelope identity, and
eight zero-byte partial artifacts. The cluster subsequently cleaned that `/tmp` tree
before a durable seal could be published; r15 does not treat its absence as evidence
and does not fabricate the lost bytes.

The immutable r10 release is commit
`ea2e9b8adc33e4f16dd4d6672bc2ca4c65b78a11`, annotated tag object
`1b1f568ae68bfac09248b9d4b03c7d9a613a8d1d`, tag
`sweep-recovery-schema5-v1.2-r10`, and namespace `schema5-v1.2-r10`. Its durable
bundle and detached checkout completed. Its r9 failure-sealer dry run passed, but
the first marker-first apply failed before completion because the wrapper expected
the r9 recorder CLI error as `ERROR: ...`; the immutable r9 CLI correctly emitted
`[schema5-evidence] ERROR: ...`. The incomplete transaction contains only its
read-only intent and declared volatile probe residue. It submitted no scheduler job
and changed no scientific result.

The tagged r11 r10-failure sealer archived the observed incomplete transaction,
failed volatile tree, and explicitly non-authoritative operator diagnostic;
reproduced the exact immutable r10 wrapper failure once with captured stdout/stderr;
archived that second incomplete transaction; and published
`$r10_prelaunch_failure_marker` last at
`prelaunch_failures/schema5-v1.2-r10-r9-sealer/PRELAUNCH_R9_SEALER_FAILURE_SEALED.json`
under protocol `schema5-v1.2-r10-r9-failure-sealer-wrapper-failure-v1`.

The immutable r11 release is commit
`6a4b100b3c2c866fe0154007fce42f90c2763864`, annotated tag object
`f9d657147ff43d2abc16389e54f8323a012b6c68`, tag
`sweep-recovery-schema5-v1.2-r11`, and namespace `schema5-v1.2-r11`. It sealed
the r10 failure above, then its corrected r9 recorder-prefix check passed. Its
independent equivalent Conda reproduction failed before marker publication because
the structural validator accepted canonical `OfflineError` blocks but rejected the
single trailing blank-line delimiter emitted around those blocks. Its incomplete
transaction contains the bound intent, exact recorder transcript, reproduced probe
...
The corrected r15 r9 verifier requires both the r10 and r11 failure seals, accepts only the exact
`[schema5-evidence] ERROR: ...` recorder stderr, runs the exact immutable r9
recorder dry/apply/idempotent sequence, archives its complete fresh reproduction
and command transcript, and performs a second equivalent empty-cache reproduction.
Its structural grammar permits at most one leading and one trailing blank-line
delimiter around complete canonical `OfflineError` blocks and rejects every other
prefix, suffix, or inter-block delimiter.
It publishes the canonical r9 marker under protocol
`schema5-v1.2-r9-prelaunch-offline-diagnostic-contract-failure-seal-v3` at
`prelaunch_failures/schema5-v1.2-r9-offline-diagnostic/PRELAUNCH_OFFLINE_DIAGNOSTIC_FAILURE_SEALED.json`.
The immutable r12 release is commit
`a91616c412b9e36a87d8232511a1ca025f8f79de`, annotated tag object
`0615b17e4027d344fed53da8259345a2383bd33f`, tag
`sweep-recovery-schema5-v1.2-r12`, and namespace `schema5-v1.2-r12`. It sealed the
r11 failure and published the complete canonical r9 transaction. Its independent
verifier then rejected that marker because the marker's transcript binding correctly
contained the full file reference (`path`, `sha256`, `size`, `mode`, `link_count`,
and `transcript_id`) while the verifier reconstructed only the four-field subset
without `mode` and `link_count`. The reproduced probe state and transcript state were
identical. No scheduler job or scientific mutation followed.

The tagged r15 r12-failure sealer replays the immutable r12 verifier read-only,
requires its exact exit-2 diagnostic, proves that the only mismatch is the complete
file-reference key set, proves the corrected same-protocol r15 verifier accepts the
unchanged canonical r9 marker, preserves a read-only copy of that transaction, and
publishes `$r12_prelaunch_failure_marker` last under protocol
`schema5-v1.2-r12-r9-verifier-file-reference-failure-v1`. It never regenerates,
overwrites, or repoints the canonical r9 evidence.

The immutable r13 release is commit
`bb692d908a04e44567a19753340b7947827476ab`, annotated tag object
`9b50e98034ca0c6f269d5cd6360c47a4864bbc1d`, tag
`sweep-recovery-schema5-v1.2-r13`, and namespace `schema5-v1.2-r13`. Its durable
bundle, detached checkout, and sealed Conda toolchain completed. The first exact r3
broken-link probe published its expected envelope. The exact empty-cache offline
probe then emitted a complete Conda 25.11 diagnostic with one leading and one
trailing blank-line delimiter; the immutable r13 verifier accepted the leading
delimiter but rejected the trailing delimiter before publishing the offline
envelope. No scheduler job or scientific result mutation followed.

The tagged r15 r13-failure sealer preserves the unchanged failed probe tree,
reproduces the exact immutable r13 recorder failure once, binds raw stdout/stderr,
proves that immutable r13 accepts the same diagnostic after removing exactly one
final newline, and proves that r15 accepts the untouched diagnostic while continuing
to reject two trailing blank delimiters. It publishes
...
written by a corrected sealer would be rejected by r15's own tagged verifier — the r12
...
The r15 renderer and every sentinel bind and reverify all twelve historical failure
...
- the r12 failure sealer dry run proves the exact immutable-verifier key-set defect,
  unchanged canonical r9 evidence, corrected same-protocol acceptance, and zero
  scheduler/scientific mutation;
- the r13 failure sealer dry run proves the exact bounded trailing-delimiter defect,
  unchanged failed probe tree, durable r13 release/toolchain identity, scheduler
  quiescence, and zero scientific mutation;
...
The r15 production prerequisite and environment-capture job accept only Conda
...
## Freeze and test the r15 source
...
the distinct `release_source_checkout_v1_2_r15` path and requires that path to be
...
Seal the deterministic r4 toolchain failure from the exact tagged r15 checkout before
...
Seal the deterministic r5 prelaunch CLI failure from the exact tagged r15 checkout.
...
r15 checkout. The dry run validates the one-field mismatch without executing the
...
r15 checkout. The dry run validates all historical inputs without cloning or running
...
r15 checkout. The dry run validates all historical inputs without executing either
...
Reverify the already sealed deterministic r10 wrapper failure with the corrected
successor-aware verifier from the exact tagged r15 checkout. Both apply calls are
verification-only and may not alter its marker or archives:

```bash
r10_failure_sealer="$pilot_checkout/scripts/seal_schema5_r10_prelaunch_failure.py"
r10_failure_seal=(
  "$dev_python" -I "$r10_failure_sealer" seal
  --evidence-root "$r10_prelaunch_failure_root"
  --recovery-root "$recovery"
  --scheduler-user "$slurm_user"
)
"${r10_failure_seal[@]}"
"${r10_failure_seal[@]}" --apply
"${r10_failure_seal[@]}" --apply
"$dev_python" -I "$r10_failure_sealer" verify \
  --evidence-root "$r10_prelaunch_failure_root" \
  --recovery-root "$recovery"
test -f "$r10_prelaunch_failure_marker"
test ! -L "$r10_prelaunch_failure_marker"
test ! -w "$r10_prelaunch_failure_marker"
```

Reverify the already sealed deterministic r11 equivalent-diagnostic delimiter
failure from the exact tagged r15 checkout. Both apply calls are verification-only
and may not alter its marker or archives:

```bash
r11_failure_sealer="$pilot_checkout/scripts/seal_schema5_r11_prelaunch_failure.py"
r11_failure_seal=(
  "$dev_python" -I "$r11_failure_sealer" seal
  --evidence-root "$r11_prelaunch_failure_root"
  --recovery-root "$recovery"
  --scheduler-user "$slurm_user"
)
"${r11_failure_seal[@]}"
"${r11_failure_seal[@]}" --apply
"${r11_failure_seal[@]}" --apply
"$dev_python" -I "$r11_failure_sealer" verify \
  --evidence-root "$r11_prelaunch_failure_root" \
  --recovery-root "$recovery"
test -f "$r11_prelaunch_failure_marker"
test ! -L "$r11_prelaunch_failure_marker"
test ! -w "$r11_prelaunch_failure_marker"
```

Reverify the unchanged canonical r9 offline-diagnostic failure with the corrected
same-protocol verifier from the exact tagged r15 checkout. The r9 marker was already
published by immutable r12; r15 may only verify this unchanged transaction:

```bash
r9_failure_sealer="$pilot_checkout/scripts/seal_schema5_r9_prelaunch_failure.py"
"$dev_python" -I "$r9_failure_sealer" verify \
  --evidence-root "$r9_prelaunch_failure_root" \
  --recovery-root "$recovery" \
  --scheduler-user "$slurm_user"
test -f "$r9_prelaunch_failure_marker"
test ! -L "$r9_prelaunch_failure_marker"
test ! -w "$r9_prelaunch_failure_marker"
```

Seal immutable r12's deterministic transcript-file-reference verifier failure from
the exact tagged r15 checkout. The dry run is read-only; the first apply executes
only the immutable r12 verifier replay, copies the unchanged canonical transaction
into durable evidence, and publishes the marker last. The repeated apply is
verification-only:

```bash
r12_failure_sealer="$pilot_checkout/scripts/seal_schema5_r12_prelaunch_failure.py"
r12_failure_seal=(
  "$dev_python" -I "$r12_failure_sealer" seal
  --evidence-root "$r12_prelaunch_failure_root"
  --recovery-root "$recovery"
  --scheduler-user "$slurm_user"
)
"${r12_failure_seal[@]}"
"${r12_failure_seal[@]}" --apply
"${r12_failure_seal[@]}" --apply
"$dev_python" -I "$r12_failure_sealer" verify \
  --evidence-root "$r12_prelaunch_failure_root" \
  --recovery-root "$recovery" \
  --scheduler-user "$slurm_user"
test -f "$r12_prelaunch_failure_marker"
test ! -L "$r12_prelaunch_failure_marker"
test ! -w "$r12_prelaunch_failure_marker"
```

Seal immutable r13's deterministic r3 offline-diagnostic delimiter failure from the
exact tagged r15 checkout. The dry run is read-only; the first apply performs one
exact reproduction, archives both observed and reproduced probe trees, and publishes
the marker last. The repeated apply is verification-only:

```bash
r13_failure_sealer="$pilot_checkout/scripts/seal_schema5_r13_prelaunch_failure.py"
r13_failure_seal=(
  "$dev_python" -I "$r13_failure_sealer" seal
  --evidence-root "$r13_prelaunch_failure_root"
  --recovery-root "$recovery"
  --scheduler-user "$slurm_user"
)
"${r13_failure_seal[@]}"
"${r13_failure_seal[@]}" --apply
"${r13_failure_seal[@]}" --apply
"$dev_python" -I "$r13_failure_sealer" verify \
  --evidence-root "$r13_prelaunch_failure_root" \
  --recovery-root "$recovery" \
  --scheduler-user "$slurm_user"
test -f "$r13_prelaunch_failure_marker"
test ! -L "$r13_prelaunch_failure_marker"
test ! -w "$r13_prelaunch_failure_marker"
```

Provision and verify the r15-only Conda toolchain from the exact tagged checkout.
...
Follow [SCHEMA5_R15_CONDA_TOOLCHAIN.md](SCHEMA5_R15_CONDA_TOOLCHAIN.md) for the complete
link, inode, offline-probe, and crash-quarantine contract.

Produce the two r3 prelaunch-failure envelopes only after the r15 toolchain above has
...
the tagged r15 provisioner in-process. That provisioner performs the complete
marker/runtime/inventory binding without executing the sealed toolchain; only after
that independent check may the exact probe command launch the sealed r15 Python or
Conda executable.
...
with the sealed r15 Python, `-I`, and `-B`. It reads, but never executes, the recorded
...
only the sealed r15 Conda and clones that same sealed base into the declared temporary
destination. Its package cache must exist and be completely empty before the first
apply. Conda 25.11 may emit its pinned clone header and terminal progress grammar on
stdout and one or more complete canonical `OfflineError` remote-fetch blocks on
stderr in nondeterministic order. The recorder accepts only that bounded structural
grammar: foreign prefixes/suffixes, non-Conda URLs, partial blocks, success, a
traceback, or an unrecognized progress stream fail closed. This supersedes the r9
single-block/empty-stdout defect without weakening the offline failure classification.

```bash
r15_probe_python="$conda_toolchain_root/base/bin/python"
r15_probe_conda="$conda_toolchain_root/base/bin/conda"
r15_sealer="$pilot_checkout/scripts/seal_recovery_evidence.py"
...
r3_probe_root="/tmp/schema5-r3-prelaunch-${slurm_user}-r15"
...
r3_broken_probe=(
  "$dev_python" -I -B "$r15_sealer" record-prelaunch-attempt
  ...
  --input-root tagged-r15-release-checkout "$pilot_checkout"
  --input-root sealed-r15-conda-toolchain "$conda_toolchain_root"
  --input-root recorded-shared-conda-base "$recorded_shared_conda_base"
  --write-root "$r3_probe_root"
)
r3_broken_command=(
  "$r15_probe_python" -I -B "$r3_runtime_identity"
...
r3_offline_command=(
  "$r15_probe_conda" create --yes --offline
...
r3_prelaunch_seal=(
  "$dev_python" -I -B "$r15_sealer" seal-prelaunch-failure
...
rerunning either failure. The recorder inventories the tagged r3 checkout, tagged r15
checkout, sealed r15 toolchain, and—only for the runtime-identity probe—the recorded
...
sealed root and the exact tagged r15 verifier. This is a hard render prerequisite:

```bash
"$dev_python" -I -B "$r15_sealer" \
...
pilot_sbatch="$recovery/jobs/schema5-v1.2-r15-materialization-pilot.sbatch"
pilot_receipt="$pilot_sbatch.receipt.json"
pilot_logs="$recovery/logs/materialization-pilot-r15"
...
protected_canary_root="$recovery/protected_capacity/schema5-v1.2-r15"
...
effective_fleet_root="$recovery/effective-fleet-schema5-v1.2-r15"
...
`schema5-v1.2-r15-effective-fleet-materialization-v1`. Its create-once intent binds
...
`schema5-v1.2-r15` namespace. It uses a semantic canonical-JSON `marker_id`; the
...
`schema5-v1.2-r15-protected-capacity-v4` and capacity source
...
protocol `schema5-v1.2-r15-external-watchdog-v1`, additionally binds a
...
watchdog_vm_stage=/var/tmp/schema5-watchdog-v1.2-r15
...
After publishing the annotated r15 tag, exact remote branch, and durable Git marker,
but before rendering the 43-job chain, require all of the following:

- the clean detached r15 pilot checkout resolves to the exact annotated tag object and
...
- the immutable r9 prelaunch seal independently verifies the reproduced accepted
  broken-link envelope, archived reproduced partial offline tree, exact historical
  recorder transcript and validator rejection, structurally canonical equivalent
  diagnostic, valid release-local toolchain, empty scheduler namespace, and zero
  schema-5 result mutations;
- the genuine r15 composite Slurm canary verifies at its fresh canonical root;
...
pre-render because they bind the annotated r15 tag and commit. Bootstrap-watchdog
...
chain_receipt="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R15_SUBMISSION.json"
...
launch_complete="$recovery/RECOVERY_CHAIN_SCHEMA5_V1_2_R15_LAUNCHED.json"
...
bootstrap_vm_stage=/var/tmp/schema5-bootstrap-watchdog-v1.2-r15
...
isolated_manifest="$isolated_root/RECOVERY_CHAIN_SCHEMA5_V1_2_R15.json"
isolated_receipt="$isolated_root/RECOVERY_CHAIN_SCHEMA5_V1_2_R15_SUBMISSION.json"
...
' "$isolated_receipt"
test ! -e "$recovery/recovery_chain_repairs_v1_2_r15"
test ! -e "$root_release"
...
`RECOVERY_CHAIN_SCHEMA5_V1_2_R15_LAUNCHED.json` is published last. A crash after
...
`recovery_chain_stage_sentinels/schema5-v1.2-r15/gNNNN/<stage>/`. A stage observation
...
Every serving allocation also carries `#SBATCH --mail-user=mabdel03@mit.edu` and
`#SBATCH --mail-type=FAIL`. A terminal serving allocation is retained in the fleet
transaction ledger and receives exactly one transactionally identified replacement;
the replacement carries the same failure-mail contract. Missing or ambiguous
scheduler identity never authorizes a duplicate replacement: admission is held and
the five-minute monitor records a durable critical fleet alert whose email delivery
retries at 60, 300, 900, and 3,600 seconds. Controller allocations have the same
direct Slurm failure mail, exact `afterany` successors, and external-watchdog
`repair-chain` fallback. These mechanisms complement rather than replace the stage
sentinels: deterministic source/protocol/integrity defects still require a
superseding release, and only sealed scheduler/node transients receive suffix repair.
...
`schema5-v1.2-r15-bootstrap-watchdog-handoff-v1`, binding the generation, receipt,
...
marker are required
pre-render inputs at their canonical r15 paths. Rendering fails if any of those
...
prerequisite-evidence schema 13 bind the
...
read-only and publishes `SOURCE_CHECKOUT_SCHEMA5_V1_2_R15_COMPLETE.json` last; jobs
...
The r15 controller implements the complete `24 -> 96 -> 192 -> 384` rollout state
...
`schema5-v1.2-r15-client-placement-capacity-generation-v1`; it binds the immutable
...
r15 release/scientific identities, the original sealed failure and receipt, the
