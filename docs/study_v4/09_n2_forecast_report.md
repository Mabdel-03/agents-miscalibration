...
## Outputs
- `forecast/reports/<item>.<method>.json` — render: messages, manifest, `prompt_token_ids`, `anchor_tokens`, `request_id`, checkpoint pins, the `Report` dict.
...
## CLI
`python -m agents_scaling.study.forecast.run --run-id study_v4 --seal <sha> --methods IND_VOTE,DEC,CEN_FLAT --items-file <panel items> --shard K --num-shards N [--report-only] [--server-run-id …] [--checkpoint 32B --N 5 --B 4 --module A --episode-rep 0] [--parallel 4] [--wait-s 300]`. Exit 0 done / 2 no endpoint / 3 infra failures / 4 protocol error(s) recorded. Cell id of the forecast requests: `forecast.<method>.<ckpt>.x<seal[:8]>`.
