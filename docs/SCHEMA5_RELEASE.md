...
First finish and test the source, create and check out the exact tag
`sweep-recovery-schema5-v1`, and remove or quarantine ignored build artifacts so the
source checkout is clean. The materializer rejects a source checkout whose `HEAD`,
status, or source-tree hash differs from that tag. Then choose empty destination paths.
The following is a dry run because it omits `--apply`:

```bash
schema5_release_base=/orcd/data/tpoggio/001/mabdel03/agents_scaling_results/recovery/schema5-v1/releases/sweep-recovery-schema5-v1
...
python scripts/materialize_schema5_release.py materialize \
  ...
  --source-repository /orcd/data/tpoggio/001/mabdel03/agents_scaling \
