...
The current operational tag is `sweep-recovery-schema5-v1.2-r12` and its chain
namespace is `schema5-v1.2-r12`. The r2 through r11 tags, remote branches, durable
bundles/checksums, and completion markers are immutable historical evidence and must
not be moved, rewritten, or copied into r12 paths. The deterministic r2 live-canary
...
`#!/usr/bin/env python`. The r4 verifier rejected that fallback. The tagged r12
...
`AttributeError` before any envelope, scheduler job, or scientific mutation. The r12
...
`portable_shebang` field while the recorder's exact field set omitted it. The r12
...
scheduler job, or scientific mutation followed. The r12 failure sealer reproduces
...
mutation followed. The r12 failure sealer binds both exact failed commands, the empty
...
cardinality. No offline envelope or scientific mutation followed. The r12 sealer
...
exact r9 rejection and r12 structural acceptance, and publishes
...
protected-capacity roots are new. Every newly created r12 artifact uses a
`schema5-v1.2-r12-*` protocol identity and binds the r12 tag and namespace. An r2
or r3/r4/r5/r6/r7/r8/r9/r10/r11 protocol is accepted only while verifying the explicitly named immutable
...
schema5_pilot="$schema5_recovery/materialization_pilots/schema5-v1.2-r12"
schema5_conda_toolchain_root="$schema5_recovery/toolchains/r12/conda"
schema5_source_package_cache=/orcd/home/002/mabdel03/.conda/pkgs
# The production DAG owns release_source_checkout_v1_2_r12 and requires it to be
# absent at render time.  Keep the prerequisite pilot checkout in its own namespace.
schema5_checkout="$schema5_recovery/materialization_pilot_source_checkout_v1_2_r12"
schema5_commit="$(git -C "$schema5_checkout" rev-parse HEAD)"
test "$(git -C "$schema5_checkout" rev-parse refs/tags/sweep-recovery-schema5-v1.2-r12^{commit})" = "$schema5_commit"
...
"$schema5_dev_python" -I "$schema5_pilot_script" run \
  ...
  --expected-tag sweep-recovery-schema5-v1.2-r12 \
  ...
  --durable-git-release-marker "$schema5_recovery/DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R12_COMPLETE.json"
```
...
```bash
schema5_pilot_sbatch="$schema5_recovery/jobs/schema5-v1.2-r12-materialization-pilot.sbatch"
schema5_pilot_receipt="$schema5_pilot_sbatch.receipt.json"
schema5_pilot_logs="$schema5_recovery/logs/materialization-pilot-r12"
...
`sweep-recovery-schema5-v1.2-r12`, and make the fresh
`release_source_checkout_v1_2_r12` checkout containing only that tag's tracked files.
