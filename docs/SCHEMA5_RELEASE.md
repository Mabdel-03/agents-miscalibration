...
The current operational tag is `sweep-recovery-schema5-v1.2-r15` and its chain
namespace is `schema5-v1.2-r15`. The r2 through r13 tags, remote branches, durable
bundles/checksums, and completion markers are immutable historical evidence and must
not be moved, rewritten, or copied into r15 paths. The deterministic r2 live-canary
...
`#!/usr/bin/env python`. The r4 verifier rejected that fallback. The tagged r15
...
`AttributeError` before any envelope, scheduler job, or scientific mutation. The r15
...
`portable_shebang` field while the recorder's exact field set omitted it. The r15
...
scheduler job, or scientific mutation followed. The r15 failure sealer reproduces
...
mutation followed. The r15 failure sealer binds both exact failed commands, the empty
...
cardinality. No offline envelope or scientific mutation followed. The r15 sealer
...
exact r9 rejection and r15 structural acceptance, and publishes
...
scientific mutation followed. The r15 sealer preserves the unchanged canonical r9
transaction, proves both the immutable r12 rejection and corrected same-protocol r15
...
delimiter. It submitted no scheduler job and mutated no scientific result. The r15
sealer archives the original and exact reproduced probe trees, binds raw output,
proves immutable r13 rejection and r15 acceptance, and publishes
...
The r15 recovery renderer must independently verify and bind all twelve exact historical
seals before render and submission. All r15 pilot, canary, source-checkout, batch-job/log, and
protected-capacity roots are new. Every newly created r15 artifact uses a
`schema5-v1.2-r15-*` protocol identity and binds the r15 tag and namespace. An r2
...
schema5_pilot="$schema5_recovery/materialization_pilots/schema5-v1.2-r15"
schema5_conda_toolchain_root="$schema5_recovery/toolchains/r15/conda"
schema5_source_package_cache=/orcd/home/002/mabdel03/.conda/pkgs
# The production DAG owns release_source_checkout_v1_2_r15 and requires it to be
# absent at render time.  Keep the prerequisite pilot checkout in its own namespace.
schema5_checkout="$schema5_recovery/materialization_pilot_source_checkout_v1_2_r15"
schema5_commit="$(git -C "$schema5_checkout" rev-parse HEAD)"
test "$(git -C "$schema5_checkout" rev-parse refs/tags/sweep-recovery-schema5-v1.2-r15^{commit})" = "$schema5_commit"
...
"$schema5_dev_python" -I "$schema5_pilot_script" run \
  ...
  --expected-tag sweep-recovery-schema5-v1.2-r15 \
  ...
  --durable-git-release-marker "$schema5_recovery/DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R15_COMPLETE.json"
```
...
```bash
schema5_pilot_sbatch="$schema5_recovery/jobs/schema5-v1.2-r15-materialization-pilot.sbatch"
schema5_pilot_receipt="$schema5_pilot_sbatch.receipt.json"
schema5_pilot_logs="$schema5_recovery/logs/materialization-pilot-r15"
...
`sweep-recovery-schema5-v1.2-r15`, and make the fresh
`release_source_checkout_v1_2_r15` checkout containing only that tag's tracked files.
