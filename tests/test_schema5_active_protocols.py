...
def test_fresh_schema5_artifact_protocols_identify_r14() -> None:
    protocols = {
...
def test_active_protocol_producers_and_consumers_are_atomic() -> None:
    assert (
        ...
        == readiness.R14_PROTOCOL
        == recovery_verifier.R14_PROTOCOL
        == "schema5-v1.2-r14-recovery-chain"
    )
    assert (
        control.SMOKE_ATTEMPT_BINDING_PROTOCOL
        == smoke_init.ATTEMPT_BINDING_PROTOCOL
...
def test_r5_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r5" in value
                    or "sweep-recovery-schema5-v1.2-r5" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
...
def test_r7_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r7" in value
                    or "sweep-recovery-schema5-v1.2-r7" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
...
def test_r8_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r8" in value
                    or "sweep-recovery-schema5-v1.2-r8" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
...
def test_r9_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r9" in value
                    or "sweep-recovery-schema5-v1.2-r9" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
...
def test_r10_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r10" in value
                    or "sweep-recovery-schema5-v1.2-r10" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/seal_schema5_r10_prelaunch_failure.py",
...
def test_r11_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r11" in value
                    or "sweep-recovery-schema5-v1.2-r11" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/seal_schema5_r11_prelaunch_failure.py",
...
def test_r12_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r12" in value
                    or "sweep-recovery-schema5-v1.2-r12" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
...
def test_r13_literals_are_confined_to_immutable_failure_history() -> None:
    root = Path(__file__).resolve().parents[1]
    observed_sources: set[str] = set()
    for directory in ("scripts", "slurm", "src"):
        for path in sorted((root / directory).rglob("*.py")):
            ...
            for node in ast.walk(tree):
                value = node.value if isinstance(node, ast.Constant) else None
                if isinstance(value, str) and (
                    "schema5-v1.2-r13" in value
                    or "sweep-recovery-schema5-v1.2-r13" in value
                ):
                    observed_sources.add(relative)
    assert observed_sources == {
        "scripts/render_schema5_recovery_chain_v12.py",
