...
def test_fresh_schema5_artifact_protocols_identify_r8() -> None:
    protocols = {
    ...
    assert protocols
    assert all("schema5-v1.2-r8-" in protocol for protocol in protocols)
...
def test_active_protocol_producers_and_consumers_are_atomic() -> None:
    assert (
        ...
        == readiness.R8_PROTOCOL
        == recovery_verifier.R8_PROTOCOL
        == "schema5-v1.2-r8-recovery-chain"
    )
    assert (
        control.SMOKE_ATTEMPT_BINDING_PROTOCOL
        == smoke_init.ATTEMPT_BINDING_PROTOCOL
