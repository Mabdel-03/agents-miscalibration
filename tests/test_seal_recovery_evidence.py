...
def _r3_probe_paths(
    ...
    r4_checkout = recovery / evidence._R4_RELEASE_CHECKOUT_DIRECTORY
    toolchain = recovery / evidence._R4_TOOLCHAIN_RELATIVE_ROOT
    temporary = root / "schema5-r3-prelaunch-probe"
    expected_paths = {
        "tagged-r3-release-checkout": r3_checkout,
        "tagged-r4-release-checkout": r4_checkout,
        "sealed-r4-conda-toolchain": toolchain,
    }
...
def _record_probe_fixture(
    ...
    for path in (r3_checkout, r4_checkout, toolchain):
...
def test_record_r3_prelaunch_attempt_brackets_immutable_verification_with_inventories(
    ...
    toolchain = dict(kwargs["input_roots"])["sealed-r4-conda-toolchain"]
...
def test_r3_probe_immutable_input_gate_binds_tags_sources_and_toolchain(
    ...
    for checkout in (r3_checkout, r4_checkout):
    ...
    r4_sealer = r4_checkout / evidence._R4_SEALER_RELATIVE_PATH
    provisioner = (
        r4_checkout / evidence._R4_TOOLCHAIN_PROVISIONER_RELATIVE_PATH
    )
    r4_sealer.write_text("exact r4 sealer\n", encoding="utf-8")
    provisioner.write_text("exact r4 provisioner\n", encoding="utf-8")
    ...
    monkeypatch.setattr(evidence, "__file__", str(r4_sealer))
    monkeypatch.setattr(
