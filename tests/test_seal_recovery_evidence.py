...
def _r3_probe_paths(
    ...
    r11_checkout = recovery / evidence._R11_RELEASE_CHECKOUT_DIRECTORY
    toolchain = recovery / evidence._R11_TOOLCHAIN_RELATIVE_ROOT
    temporary = root / "schema5-r3-prelaunch-probe"
    expected_paths = {
        "tagged-r3-release-checkout": r3_checkout,
        "tagged-r11-release-checkout": r11_checkout,
        "sealed-r11-conda-toolchain": toolchain,
    }
...
def _record_probe_fixture(
    ...
    for path in (r3_checkout, r11_checkout, toolchain):
...
def test_record_r3_prelaunch_attempt_brackets_immutable_verification_with_inventories(
    ...
    toolchain = dict(kwargs["input_roots"])["sealed-r11-conda-toolchain"]
...
def test_r3_probe_immutable_input_gate_binds_tags_sources_and_toolchain(
    ...
    for checkout in (r3_checkout, r11_checkout):
    ...
    r11_sealer = r11_checkout / evidence._R11_SEALER_RELATIVE_PATH
    provisioner = (
        r11_checkout / evidence._R11_TOOLCHAIN_PROVISIONER_RELATIVE_PATH
    )
    r11_sealer.write_text("exact r11 sealer\n", encoding="utf-8")
    provisioner.write_text("exact r11 provisioner\n", encoding="utf-8")
    ...
    monkeypatch.setattr(evidence, "__file__", str(r11_sealer))
    monkeypatch.setattr(
...
def test_verify_r3_prelaunch_failure_seal_rejects_release_substitution(
    ...
    intent["release"]["release_tag"] = "sweep-recovery-schema5-v1.2-r11"
    ...
    marker["release"]["release_tag"] = "sweep-recovery-schema5-v1.2-r11"
