...
def test_recovery_chain_documents_transactional_failfast_stage_observers() -> None:
    ...
    assert (
        "recovery_chain_stage_sentinels/schema5-v1.2-r14/gNNNN/<stage>/"
        in text
    )
...
def test_recovery_chain_documents_prelaunch_bootstrap_resurrection_gate() -> None:
    ...
    assert "schema5-v1.2-r14-bootstrap-watchdog-handoff-v1" in text
...
def test_materialization_pilot_uses_only_the_exact_tagged_script() -> None:
    ...
    assert section.count(
        ...
        'DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R14_COMPLETE.json"'
    ) == 2
...
def test_chain_renderer_receives_canonical_python_paths() -> None:
    ...
    assert "RECOVERY_CHAIN_SCHEMA5_V1_2_R14_SUBMISSION.json" in render_section
...
def test_protected_capacity_uses_marker_last_effective_fleet_materializer() -> None:
    ...
    assert (
        "schema5-v1.2-r14-effective-fleet-materialization-v1"
        in section
    )
...
def test_durable_release_and_watchdog_commands_are_exact_and_fail_closed() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    freeze = text[
        text.index("## Freeze and test the r14 source")
        : text.index("## Required two-prefix materialization pilot")
    ]
    watchdog = text[
...
def test_r14_uses_fresh_operational_paths_and_binds_sealed_r2_r3_lineage() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    release = RELEASE_GUIDE.read_text(encoding="utf-8")

    assert "# Schema-5 v1.2-r14 recovery and production runbook" in text
    assert "tag=sweep-recovery-schema5-v1.2-r14" in text
    assert "chain namespace is `schema5-v1.2-r14`" in text
    assert "durable_commit_ref=refs/heads/schema5-v1.2-r14" in text
    assert (
    ...
    assert (
        ...
        'DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R14_COMPLETE.json"' in text
    )
    for path in (
        "materialization_pilot_source_checkout_v1_2_r14",
        "materialization_pilots/schema5-v1.2-r14",
        "slurm_canaries/schema5-v1.2-r14",
        "jobs/schema5-v1.2-r14-materialization-pilot.sbatch",
        "logs/materialization-pilot-r14",
        "protected_capacity/schema5-v1.2-r14",
        "recovery_chain_stage_sentinels/schema5-v1.2-r14",
    ):
        assert path in text

    lineage = text[
        text.index("## Immutable r2–r13 to r14 lineage")
    ...
    assert "Operational artifact basenames are r14-specific" in lineage
    assert "chain_namespace=schema5-v1.2-r14" in lineage
    ...
    assert "sweep-recovery-schema5-v1.2-r14" in release
    assert "canary_failures/schema5-v1.2-r2/CANARY_FAILURE_SEALED.json" in release
    assert "jobs/schema5-v1.2-r14-materialization-pilot.sbatch" in release
    assert "logs/materialization-pilot-r14" in release
...
def test_r12_verifier_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    ...
    section = text[
        ...
        : text.index("Provision and verify the r14-only Conda toolchain")
    ]
...
def test_r3_failure_producer_independently_verifies_before_toolchain_use() -> None:
    ...
    assert '"$dev_python" -I -B "$r14_sealer" seal-prelaunch-failure' in section
    assert '"$dev_python" -I -B "$r14_sealer" \\\n' in section
    assert '"$r14_probe_python" -I -B "$r3_runtime_identity"' in section
    assert '"$r14_probe_python" -I -B "$r14_sealer"' not in section
    assert "imports\nthe tagged r14 provisioner in-process" in section
...
def test_r14_docs_use_sealed_toolchain_cache_and_current_schema_contracts() -> None:
    runbook = RUNBOOK.read_text(encoding="utf-8")
    release = RELEASE_GUIDE.read_text(encoding="utf-8")
    toolchain = RUNBOOK.with_name("SCHEMA5_R14_CONDA_TOOLCHAIN.md").read_text(
        encoding="utf-8"
    )
    ...
    assert (
        "materialization_pilot_source_checkout_v1_2_r14" in toolchain
    )
...
def test_release_gates_follow_executable_tag_order() -> None:
    ...
    assert text.index("## Freeze and test the r14 source") < text.index(
...
def test_retired_r1_record_preserves_historical_r2_successor() -> None:
    ...
    assert "requires the v1.2-r14" not in text
