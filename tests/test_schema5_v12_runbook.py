...
def test_recovery_chain_documents_transactional_failfast_stage_observers() -> None:
    ...
    assert (
        "recovery_chain_stage_sentinels/schema5-v1.2-r11/gNNNN/<stage>/"
        in text
    )
    assert "explicitly has no repair authority" in text


def test_storage_admission_is_seven_tib_without_weakening_live_floor() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    normalized = " ".join(text.split())
    config = json.loads(
        (
            RUNBOOK.parents[1] / "configs" / "schema5_monitoring.v1.json"
        ).read_text(encoding="utf-8")
    )

    assert "storage_admission_minimum_free_bytes=7696581394432" in text
    assert "operator-authorized pre-qualification threshold is 7 TiB" in text
    assert (
        "cluster subsequently cleaned that `/tmp` tree before a durable seal "
        "could be published"
    ) in normalized
    assert "does not treat its absence as evidence" in normalized
    assert 'test "$storage_free_bytes" -ge "$storage_admission_minimum_free_bytes"' in text
    assert 'test "$storage_free_inodes" -ge 1000000' in text
    assert 'test "$storage_inode_use_percent" -le 95' in text
    assert (
...
def test_fleet_failures_have_direct_email_and_transactional_fallback() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")

    assert "`#SBATCH --mail-user=mabdel03@mit.edu`" in text
    assert "`#SBATCH --mail-type=FAIL`" in text
    assert "exactly one transactionally identified replacement" in text
    assert "external-watchdog\n`repair-chain` fallback" in text
    assert "60, 300, 900, and 3,600 seconds" in text
...
def test_recovery_chain_documents_prelaunch_bootstrap_resurrection_gate() -> None:
    ...
    assert "schema5-v1.2-r11-bootstrap-watchdog-handoff-v1" in text
...
def test_materialization_pilot_uses_only_the_exact_tagged_script() -> None:
    ...
    assert section.count(
        ...
        'DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R11_COMPLETE.json"'
    ) == 2
...
def test_chain_renderer_receives_canonical_python_paths() -> None:
    ...
    assert "RECOVERY_CHAIN_SCHEMA5_V1_2_R11_SUBMISSION.json" in render_section
...
def test_protected_capacity_uses_marker_last_effective_fleet_materializer() -> None:
    ...
    assert (
        "schema5-v1.2-r11-effective-fleet-materialization-v1"
        in section
    )
...
def test_durable_release_and_watchdog_commands_are_exact_and_fail_closed() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    freeze = text[
        text.index("## Freeze and test the r11 source")
        : text.index("## Required two-prefix materialization pilot")
    ]
    watchdog = text[
...
def test_r11_uses_fresh_operational_paths_and_binds_sealed_r2_r3_lineage() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    release = RELEASE_GUIDE.read_text(encoding="utf-8")

    assert "# Schema-5 v1.2-r11 recovery and production runbook" in text
    assert "tag=sweep-recovery-schema5-v1.2-r11" in text
    assert "chain namespace is `schema5-v1.2-r11`" in text
    assert "durable_commit_ref=refs/heads/schema5-v1.2-r11" in text
    assert (
        "schema5-v1.2-r11-client-placement-capacity-generation-v1"
        in text
    )
    assert (
    ...
    assert (
        ...
        'DURABLE_GIT_RELEASE_SCHEMA5_V1_2_R11_COMPLETE.json"' in text
    )
    for path in (
        "materialization_pilot_source_checkout_v1_2_r11",
        "materialization_pilots/schema5-v1.2-r11",
        "slurm_canaries/schema5-v1.2-r11",
        "jobs/schema5-v1.2-r11-materialization-pilot.sbatch",
        "logs/materialization-pilot-r11",
        "protected_capacity/schema5-v1.2-r11",
        "recovery_chain_stage_sentinels/schema5-v1.2-r11",
    ):
        assert path in text

    lineage = text[
        text.index("## Immutable r2–r10 to r11 lineage")
    ...
    assert "Operational artifact basenames are r11-specific" in lineage
    assert "chain_namespace=schema5-v1.2-r11" in lineage
    ...
    assert "fa50d619579d6ffde9ab768d342bb65f1b98d844" in lineage
    assert "51267bb1bbe59833bfadc96b4d3ad74355ce1d39" in lineage
    assert "schema5-v1.2-r5-prelaunch-cli-dispatch-failure-seal-v1" in lineage
    assert "PRELAUNCH_CLI_FAILURE_SEALED.json" in lineage
    assert "9b40414a0d90f569cf021ac152503746bedda6c7" in lineage
    assert "e6b9c2f2548f7ef58ad5163b65a25b164013a4e6" in lineage
    assert (
        "schema5-v1.2-r6-prelaunch-toolchain-binding-failure-seal-v1"
        in lineage
    )
    assert "PRELAUNCH_TOOLCHAIN_BINDING_FAILURE_SEALED.json" in lineage
    assert "acc0beb98cfbb5017d5d5b5e60567aa43229e248" in lineage
    assert "95f8f3852498c4ab8e3026f29c183ecdd58678f4" in lineage
    assert (
        "schema5-v1.2-r7-prelaunch-git-index-refresh-failure-seal-v1"
        in lineage
    )
    assert "PRELAUNCH_GIT_INDEX_REFRESH_FAILURE_SEALED.json" in lineage
    assert "ed18bb969c6632d2b2643024064a2b4d1b63f6de" in lineage
    assert "c31692215b98c176bdd138109b0c2b6ec0a63e1f" in lineage
    assert (
        "schema5-v1.2-r8-prelaunch-r3-probe-import-failure-seal-v1"
        in lineage
    )
    assert "PRELAUNCH_R3_PROBE_IMPORT_FAILURE_SEALED.json" in lineage
    assert "f134bba109ce77beca60584a5f589361856f2301" in lineage
    assert "17b0386800c70fb9e72d090c7b8e1c3ddf9c7b52" in lineage
    assert (
    ...
    assert "sweep-recovery-schema5-v1.2-r11" in release
    assert "canary_failures/schema5-v1.2-r2/CANARY_FAILURE_SEALED.json" in release
    assert "jobs/schema5-v1.2-r11-materialization-pilot.sbatch" in release
    assert "logs/materialization-pilot-r11" in release
    assert "schema5-v1.2-r5-cli/PRELAUNCH_CLI_FAILURE_SEALED.json" in release
    assert (
        "schema5-v1.2-r6-toolchain-binding/"
        "PRELAUNCH_TOOLCHAIN_BINDING_FAILURE_SEALED.json"
        in release
    )
    assert (
        "schema5-v1.2-r7-git-index-refresh/"
        "PRELAUNCH_GIT_INDEX_REFRESH_FAILURE_SEALED.json"
        in release
    )
    assert (
        "schema5-v1.2-r8-r3-probe-import/"
        "PRELAUNCH_R3_PROBE_IMPORT_FAILURE_SEALED.json"
        in release
    )
    assert (
        "schema5-v1.2-r9-offline-diagnostic/"
        "PRELAUNCH_OFFLINE_DIAGNOSTIC_FAILURE_SEALED.json"
        in release
    )
    assert "ea2e9b8adc33e4f16dd4d6672bc2ca4c65b78a11" in lineage
    assert "1b1f568ae68bfac09248b9d4b03c7d9a613a8d1d" in lineage
    assert "PRELAUNCH_R9_SEALER_FAILURE_SEALED.json" in lineage
    assert "schema5-v1.2-r10-r9-failure-sealer-wrapper-failure-v1" in lineage


def test_r5_cli_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    section = text[
        text.index("Seal the deterministic r5 prelaunch CLI failure")
        : text.index(
            "Seal the deterministic r6 prelaunch toolchain-binding failure"
        )
    ]

    assert 'r5_failure_sealer="$pilot_checkout/scripts/' in section
    assert "seal_schema5_r5_prelaunch_failure.py" in section
    assert section.count('"${r5_failure_seal[@]}"') == 3
    assert section.count('"${r5_failure_seal[@]}" --apply') == 2
    assert '"$dev_python" -I "$r5_failure_sealer" verify' in section
    assert '--evidence-root "$r5_prelaunch_failure_root"' in section
    assert '--recovery-root "$recovery"' in section


def test_r6_binding_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    section = text[
        text.index(
            "Seal the deterministic r6 prelaunch toolchain-binding failure"
        ...
        : text.index(
            "Seal the deterministic r7 prelaunch Git-index-refresh failure"
        )
    ]

    assert 'r6_failure_sealer="$pilot_checkout/scripts/' in section
    assert "seal_schema5_r6_prelaunch_failure.py" in section
    assert section.count('"${r6_failure_seal[@]}"') == 3
    assert section.count('"${r6_failure_seal[@]}" --apply') == 2
    assert '"$dev_python" -I "$r6_failure_sealer" verify' in section
    assert '--evidence-root "$r6_prelaunch_failure_root"' in section
    assert '--recovery-root "$recovery"' in section


def test_r7_git_index_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    section = text[
        text.index(
            "Seal the deterministic r7 prelaunch Git-index-refresh failure"
    ...
    assert 'r7_failure_sealer="$pilot_checkout/scripts/' in section
    assert "seal_schema5_r7_prelaunch_failure.py" in section
    assert section.count('"${r7_failure_seal[@]}"') == 3
    assert section.count('"${r7_failure_seal[@]}" --apply') == 2
    assert '"$dev_python" -I "$r7_failure_sealer" verify' in section
    assert '--evidence-root "$r7_prelaunch_failure_root"' in section
    assert '--recovery-root "$recovery"' in section


def test_r8_probe_import_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    text = RUNBOOK.read_text(encoding="utf-8")
    section = text[
        text.index(
            "Seal the deterministic r8 prelaunch r3-probe import failure"
    ...
    assert 'r8_failure_sealer="$pilot_checkout/scripts/' in section
    assert "seal_schema5_r8_prelaunch_failure.py" in section
    assert section.count('"${r8_failure_seal[@]}"') == 3
    assert section.count('"${r8_failure_seal[@]}" --apply') == 2
    assert '"$dev_python" -I "$r8_failure_sealer" verify' in section
    assert '--evidence-root "$r8_prelaunch_failure_root"' in section
    assert '--recovery-root "$recovery"' in section
...
def test_r9_offline_diagnostic_failure_seal_is_dry_apply_idempotent_and_verified() -> None:
    ...
    section = text[
        ...
        : text.index("Provision and verify the r11-only Conda toolchain")
    ]
...
def test_r3_failure_producer_independently_verifies_before_toolchain_use() -> None:
    ...
    assert '"$dev_python" -I -B "$r11_sealer" seal-prelaunch-failure' in section
    assert '"$dev_python" -I -B "$r11_sealer" \\\n' in section
    assert '"$r11_probe_python" -I -B "$r3_runtime_identity"' in section
    assert '"$r11_probe_python" -I -B "$r11_sealer"' not in section
    assert "imports\nthe tagged r11 provisioner in-process" in section
...
def test_r11_docs_use_sealed_toolchain_cache_and_current_schema_contracts() -> None:
    runbook = RUNBOOK.read_text(encoding="utf-8")
    release = RELEASE_GUIDE.read_text(encoding="utf-8")
    toolchain = RUNBOOK.with_name("SCHEMA5_R11_CONDA_TOOLCHAIN.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "prerequisite-evidence schema 13" in runbook
    ...
    assert (
        "materialization_pilot_source_checkout_v1_2_r11" in toolchain
    )
...
def test_release_gates_follow_executable_tag_order() -> None:
    ...
    assert text.index("## Freeze and test the r11 source") < text.index(
...
def test_retired_r1_record_preserves_historical_r2_successor() -> None:
    ...
    assert "requires the v1.2-r11" not in text
