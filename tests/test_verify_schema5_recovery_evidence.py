...
@pytest.mark.parametrize(
    ...
    [
        ...
        (evidence.R13_PROTOCOL, "render_schema5_recovery_chain_v12"),
    ],
)
...
def test_r13_repair_receipt_dispatches_to_generation_validator(
    ...
    manifest.write_text(
        json.dumps({"protocol": evidence.R13_PROTOCOL, "chain_id": "chain"}) + "\n",
        encoding="utf-8",
    )
    parent.write_text("{}\n", encoding="utf-8")
    receipt.write_text(
        json.dumps(
            {
                "protocol": f"{evidence.R13_PROTOCOL}-repair",
                "repair_generation": 1,
...
def test_rejects_symlinked_ancestor(tmp_path: Path) -> None:
    ...
    manifest.write_text(
        json.dumps({"protocol": evidence.R13_PROTOCOL}) + "\n",
        encoding="utf-8",
    )
    manifest.chmod(0o444)
